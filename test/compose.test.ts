...
describe('compose', () => {
  it('places sprites on a canvas', () => {
    ...
    const canvas = compose().place(s, { x: 0, y: 0 }).render();
  ...
  it('computes bounding box from placed items', () => {
    const s = sprite({ palette, frames: ['x'] });
    const canvas = compose().place(s, { x: 0, y: 0 }).place(s, { x: 4, y: 4 }).render();
  ...
  it('fills background color', () => {
    const s = sprite({ palette, frames: ['x'] });
    const canvas = compose({ background: '#00ff00' }).place(s, { x: 1, y: 1 }).render();
    // Background at (0,0)
  ...
  it('handles negative coordinates', () => {
    const s = sprite({ palette, frames: ['x'] });
    const canvas = compose().place(s, { x: -2, y: -2 }).place(s, { x: 2, y: 2 }).render();
  ...
  it('respects fixed width/height', () => {
    const s = sprite({ palette, frames: ['x'] });
    const canvas = compose({ width: 10, height: 10 }).place(s, { x: 0, y: 0 }).render();
    expect(canvas.width).toBe(10);
    expect(canvas.height).toBe(10);
  });

  it('background() fluent chain sets background color', () => {
    const s = sprite({ palette, frames: ['x'] });
    const canvas = compose().background('#00ff00').place(s, { x: 1, y: 1 }).render();
    expect(canvas.getPixel(0, 0)).toEqual([0, 255, 0, 255]);
    expect(canvas.getPixel(1, 1)).toEqual([255, 0, 0, 255]);
